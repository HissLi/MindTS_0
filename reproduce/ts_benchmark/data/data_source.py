...
class LocalDataSource(DataSource):
    ...
    def load_multi_list(self, series_list: List[str], text_list: List[str]) -> NoReturn:
        logger.info("Start loading %s series in parallel", len(series_list))
        data_dict = {}
        with ThreadPoolExecutor() as executor:
            futures = [
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self._load_series, text_name)
                for text_name in text_list
            ]
        for future, text_name in zip(futures, text_list):
            text_dict[text_name] = future.result()

        logger.info("Data loading finished.")
        self.dataset.update_multi_data(data_dict, text_dict, covariate_dict)
