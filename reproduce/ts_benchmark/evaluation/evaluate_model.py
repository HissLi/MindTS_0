...
def eval_multi_model(
    model_factory: ModelFactory, series_list: list, text_list: list, evaluation_config: dict
) -> EvalResult:
    """
    Evaluate the performance of the model on time series data.
    Evaluate the model based on the provided model factory, time series list, and evaluation configuration, and return the DataFrame of the evaluation results.
    ...
    eval_backend = ParallelBackend()
    result_list = []
    for series_name, text_name in tqdm.tqdm(
        zip(series_list, text_list), desc=f"scheduling {model_factory.model_name}"
    ):
        # TODO: refactor data model to optimize communication cost in parallel mode
        result_list.append(
            eval_backend.schedule(strategy.multi_execute, (series_name, text_name, model_factory))
        )
...
def eval_mmd_model(
    ...
    for series_name, text_name in tqdm.tqdm(
        ...
        # TODO: refactor data model to optimize communication cost in parallel mode
        result_list.append(
            eval_backend.schedule(strategy.mmd_execute, (series_name, text_name, model_factory))
        )
