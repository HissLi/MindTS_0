...
from ts_benchmark.baselines.utils import (
    ...
    get_time_mark,
    anomaly_detection_multi_data_provider,
    anomaly_detection_timeMMD_data_provider,
)
...
class TransformerAdapter(ModelBase):
    def __init__(self, model_name, model_class, **kwargs):
        ...
        # self.prompt_weight=self.config.prompt_weight
        # self.d_llm = self.config.llm_dim
        # self.pred_len=self.config.seq_len
        # self.text_embedding_dim = self.config.text_emb
        # self.pool_type=self.config.pool_type
        # self.use_fullmodel=self.config.use_fullmodel
        # self.hug_token=self.config.huggingface_token
        ...
        # if self.configs.llm_model != 'Doc2Vec':
        #     if self.configs.llm_model == 'GPT2':
        #         self.gpt2_config = GPT2Config.from_pretrained('openai-community/gpt2')

        #         self.gpt2_config.num_hidden_layers = self.configs.llm_layers
        ...
        #     elif self.configs.llm_model == 'BERT':
        #         self.bert_config = BertConfig.from_pretrained('google-bert/bert-base-uncased')

        #         self.bert_config.num_hidden_layers = self.configs.llm_layers
        ...
        # self.weight1 = nn.Embedding(1, self.pred_len)
        # self.weight2 = nn.Embedding(1, self.pred_len)
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        :param covariates: Covariates of time series.
    ...
    def forecast(
        ...
        **kwargs,
        ...
        :param train: Time series data used for prediction.
        :param covariates: Covariates of time series.
    ...
    def detect_timeMMD_validate(self, vali_data, valid_data_loader, criterion):
        config = self.config
        total_loss = []
        self.model.eval()
        self.mlp.eval()
        ...
        for input, _, index in valid_data_loader:
            input = input.to(device)
            ...
            output=(1-self.prompt_weight)*output+self.prompt_weight*prompt_y
    ...
    def detect_timeMMD_fit(self, train_data: pd.DataFrame, train_text: pd.DataFrame, train_label: pd.DataFrame):
        """
        训练模型。
        ...
        self.valid_data, self.valid_data_loader = anomaly_detection_timeMMD_data_provider(
            valid_data,
            valid_text,
            batch_size=config.batch_size,
            win_size=config.seq_len,
        ...
        self.train_data, self.train_data_loader = anomaly_detection_timeMMD_data_provider(
            train_data_value,
            train_data_text,
            batch_size=config.batch_size,
            win_size=config.seq_len,
        ...
        for epoch in range(config.num_epochs):
            self.model.train()
            for i, (input, target, index) in enumerate(self.train_data_loader):
                optimizer.zero_grad()
                input = input.float().to(self.device)
                batch_text=self.train_data.get_text(index)
                prompt = [f"<|start_prompt|Make predictions about the future based on the following information: {text_info}<|<end_prompt>|>" for text_info in batch_text]
                prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024).input_ids
                prompt_embeddings = self.llm_model.get_input_embeddings()(prompt.to(self.device))
                ...
                loss.backward()
                optimizer.step()
            valid_loss = self.detect_timeMMD_validate(self.train_data, self.valid_data_loader, criterion)
            self.early_stopping(valid_loss, self.model)
            if self.early_stopping.early_stop:
                break
    ...
    def detect_timeMMD_score(self, test_data: pd.DataFrame, test_text: pd.DataFrame) -> np.ndarray:
        ...
        self.thre_data, self.thre_loader = anomaly_detection_timeMMD_data_provider(
            test_data,
            test_text,
            batch_size=config.batch_size,
            win_size=config.seq_len,
        ...
        attens_energy = []
        test_labels = []
        for i, (batch_x, batch_y, index) in enumerate(self.thre_loader):
            batch_x = batch_x.float().to(self.device)
            batch_text=self.thre_data.get_text(index).to(self.device)
            prompt = [f"<|start_prompt|Make predictions about the future based on the following information: {text_info}<|<end_prompt>|>" for text_info in batch_text]
            prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024).input_ids
            prompt_embeddings = self.llm_model.get_input_embeddings()(prompt.to(self.device))
            ...
            if self.pool_type=="avg":                
                ...
                outputs_reshaped = output#.transpose(1, 2)  
            ...
            score = torch.mean(self.anomaly_criterion(batch_x, output), dim=-1)
            score = score.detach().cpu().numpy()
            attens_energy.append(score)
            test_labels.append(batch_y)
    ...
    def detect_timeMMD_label(self, test_data: pd.DataFrame, test_text: pd.DataFrame) -> np.ndarray:
        test_data = pd.DataFrame(
            self.scaler.transform(test_data.values), columns=test_data.columns, index=test_data.index
        )
        ...
        self.test_data, self.test_data_loader = anomaly_detection_timeMMD_data_provider(
            test_data,
            test_text,
            batch_size=config.batch_size,
            win_size=config.seq_len,
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, index) in enumerate(self.train_data_loader):
                batch_x = batch_x.float().to(self.device)
                batch_text=self.train_data.get_text(index)
                prompt = [f"<|start_prompt|Make predictions about the future based on the following information: {text_info}<|<end_prompt>|>" for text_info in batch_text]
                prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024).input_ids
        ...
        attens_energy = []
        test_labels = []
        for i, (batch_x, batch_y, index) in enumerate(self.test_data_loader):
            batch_x = batch_x.float().to(self.device)
            batch_text=self.test_data.get_text(index)
            prompt = [f"<|start_prompt|Make predictions about the future based on the following information: {text_info}<|<end_prompt>|>" for text_info in batch_text]
            prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024).input_ids
            prompt_embeddings = self.llm_model.get_input_embeddings()(prompt.to(self.device)) 
            if self.use_fullmodel:
        ...
        for i, (batch_x, batch_y, index) in enumerate(self.thre_loader):
            batch_x = batch_x.float().to(self.device)
            batch_text=self.thre_data.get_text(index)
            prompt = [f"<|start_prompt|Make predictions about the future based on the following information: {text_info}<|<end_prompt>|>" for text_info in batch_text]
            prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024).input_ids
            prompt_embeddings = self.llm_model.get_input_embeddings()(prompt.to(self.device)) 
            if self.use_fullmodel:
