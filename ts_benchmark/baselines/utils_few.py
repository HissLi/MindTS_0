...
class SegLoader(object):
    def __init__(self, data, win_size, step, mode="train", sampling_rate=1):
        self.mode = mode
    ...
    def __len__(self):
        ...
        """
        if self.mode == "train":
            return int(((self.data.shape[0] - self.win_size) // self.step + 1) * self.sampling_rate)
        elif self.mode == "val":
    ...
    def __getitem__(self, index):
        index = index * self.step * self.internal
        if self.mode == "train":
...
def anomaly_detection_data_provider(data, batch_size, win_size=100, step=100, mode='train', sampling_rate=1):
    dataset = SegLoader(data, win_size, 1, mode, sampling_rate)
