...
class GlobalStorageDataPool(DataPoolImpl):
    ...
    def get_covariates(self, name: str) -> Optional[Dict]:
        """
        Gets the covariates of time series from the global storage

        NOTE the data in the global storage is considered READ-ONLY,
        DO NOT perform inplace operations on the return value.

        :param name: The name of the series to get.
        :return: The dictionary of the time series' covariates.
        """
        if self._global_dataset is None:
            raise ValueError("Data covariates is not ready.")
        return self._global_dataset.get_covariates(name)
    ...
    def get_text(self, name: str) -> Optional[pd.DataFrame]:
        ...
        if self._global_dataset is None:
            raise ValueError("Data is not ready.")
        return self._global_dataset.get_text(name)
